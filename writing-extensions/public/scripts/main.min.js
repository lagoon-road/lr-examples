    ...
    const observer = new MutationObserver(function(mutations) {
      mutations.forEach(function(mutation) {
        if (mutation.attributeName === 'lg-placeholder') {
          update({ matchValue : id, updateType : 'domReady' });
        }
      });
    });
...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  require('./road')(road).where('client').update({ matchValue: 'nav', updateType: 'domReady' });
});

},{"../extensions/httpClient":313,"../middleware/events/navigation":321,"./road":311,"babel-polyfill":5,"lr-client-renderer":1,"lr-client-router":2,"lr-core":3}],311:[function(require,module,exports){
'use strict';

var debug = require('../extensions/debug');

module.exports = function (road) {
  return road.extension('debug', debug).middleware({
  ...
  }).where('webserver').run('*', 'debug').run('*', 'statics').run('*', 'layouts.default').where('client').run('nav', 'events.navigation', 'domReady').where('webserver', 'client').run('*', 'components.navigation').run('/', 'components.home').run('/about', 'components.loading').run('/about', 'components.about').run('/error', 'components.fail').error('components.error').where('webserver').done('response');
};

},{"../extensions/debug":312,"../middleware/components/about":314,"../middleware/components/error":315,"../middleware/components/fail":316,"../middleware/components/home":317,"../middleware/components/loading":318,"../middleware/components/navigation":319,"../middleware/debug":320}],312:[function(require,module,exports){
'use strict';
...
},{"superagent":303}],314:[function(require,module,exports){
'use strict';
